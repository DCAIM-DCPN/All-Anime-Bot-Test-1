...
Output ONLY the Python code for 'executor.py'."""
        ...
        try:
            ...
            # Step 2: Poll for completion via task.listMessages (GET with query params)
            ...
            for attempt in range(max_attempts):
                ...
                # Determine current status from status_update events
                agent_status = None
                ...
                # Check for structured_output_result event (from structured_output_schema)
                for msg in reversed(messages):
                    if msg.get("type") == "structured_output_result":
                        result = msg.get("structured_output_result", {})
                        if result.get("success"):
                            return result.get("value", {}).get("executor_code")
                        else:
                            print(f"Manus structured output extraction failed: {result.get('error')}")
                            return None

                if agent_status == "stopped":
                    # Task completed but no structured output — fallback to last assistant message
                    for msg in reversed(messages):
                        if msg.get("type") == "assistant_message":
                            result_msg = msg.get("assistant_message", {}).get("content", "")
                    ...
                    print("Manus task stopped but no code found in assistant messages.")
                    return None
                elif agent_status == "error":
                    for msg in reversed(messages):
                        if msg.get("type") == "error_message":
                            err = msg.get("error_message", {})
                            print(f"Manus task error: {err.get('content', 'unknown')}")
                            break
                    return None

                print(f"Waiting for Manus... (Status: {agent_status or 'running'}, messages: {len(messages)}, attempt {attempt + 1}/{max_attempts})")
